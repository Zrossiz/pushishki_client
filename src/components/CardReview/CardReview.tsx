...
export const CardReview = ({ review }: CardReviewProps) => {
    ...
    const stringDate: string = `${date.getDay().toLocaleString().length === 1 ? `0${date.getDay()}` : date.getDay()}.${date.getMonth().toLocaleString().length === 1 ? `0${date.getMonth()}` : date.getMonth()}.${date.getFullYear()}`;
